...
def test_demo_correlation_guard_accepts_explicit_five_percent(monkeypatch):
    ...
    assert resolver.demo_max_risk_pct == 5.0


def test_demo_correlation_guard_rejects_above_five_percent(monkeypatch):
    monkeypatch.setattr(ProductionGuardResolver, "__init__", _stub_base_init)
    monkeypatch.setenv("CTRADER_DEMO_RISK_PER_TRADE_PCT", "5.01")

    with pytest.raises(
        ValueError,
        match=r"CTRADER_DEMO_RISK_PER_TRADE_PCT must be in \(0,5\]",
    ):
