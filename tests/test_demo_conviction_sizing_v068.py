...
def test_conviction_sizing_uses_full_user_approved_lot_and_risk_ranges():
    cases = (
        (_row(55.0), "BASE", 0.01, 1.0),
        (_row(65.0), "B", 0.02, 2.0),
        (_row(75.0, 0.90, 2.0), "B_PLUS", 0.03, 3.0),
        (_row(85.0, 0.95, 2.2), "A", 0.05, 5.0),
        (_row(92.0, 0.92, 2.2), "A_PLUS", 0.07, 7.0),
        (_row(97.0, 0.97, 3.0), "ELITE", 0.10, 10.0),
    )
...
def test_high_score_cannot_unlock_elite_without_coverage_and_rr():
    ...
    assert sizing.lots == 0.07
    assert sizing.risk_budget_pct == 7.0


def test_runtime_caps_can_only_reduce_conviction_sizing():
    ...
    assert sizing.risk_budget_pct == 4.0
