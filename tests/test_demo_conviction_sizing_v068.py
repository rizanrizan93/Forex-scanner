...
def test_conviction_sizing_keeps_fixed_lot_and_bounded_risk_by_quality_tier():
    cases = (
        (_row(55.0), "BASE", 0.01, 0.5),
        (_row(65.0), "B", 0.01, 1.0),
        (_row(75.0, 0.90, 2.0), "B_PLUS", 0.01, 2.0),
        (_row(85.0, 0.95, 2.2), "A", 0.01, 3.0),
        (_row(92.0, 0.92, 2.2), "A_PLUS", 0.01, 3.0),
...
def test_high_score_cannot_exceed_three_percent_even_without_elite_confirmations():
    sizing = select_demo_conviction_sizing(_row(99.0, 0.90, 2.0))
    assert sizing.tier == "A_PLUS"
    assert sizing.lots == 0.01
    assert sizing.risk_budget_pct == 3.0


def test_runtime_cap_can_reduce_risk_but_not_raise_lot_above_point_zero_one():
