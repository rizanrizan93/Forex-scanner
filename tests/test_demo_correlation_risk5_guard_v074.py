...
def test_demo_correlation_guard_enforces_three_percent_contract():
    source = (ROOT / "src/fx_scanner/demo_correlation_evidence.py").read_text(encoding="utf-8")
    assert "MAX_DEMO_RISK_PCT = 3.0" in source
    assert 'CTRADER_DEMO_RISK_PER_TRADE_PCT must be in (0,3]' in source
    assert "MAX_DEMO_RISK_PCT = 5.0" not in source


def test_fast_pipeline_requests_three_percent_point_zero_one_contract():
