...
def _row(signal_id, *, state="EXECUTION_READY", observed_at, expires_at):
    return {"id":signal_id,"state":state,"observed_at":observed_at.isoformat(),"expires_at":expires_at.isoformat()}


def test_fresh_handoff_drops_stale_ready_backlog_and_keeps_newest_fresh_rows():
    now=datetime(2026,9,7,9,0,tzinfo=UTC)
    rows=[_row("stale-1",observed_at=now-timedelta(minutes=20),expires_at=now-timedelta(minutes=15)),_row("fresh-older",observed_at=now-timedelta(seconds=120),expires_at=now+timedelta(seconds=180)),_row("fresh-newer",observed_at=now-timedelta(seconds=15),expires_at=now+timedelta(seconds=285)),_row("forming",state="SETUP_FORMING",observed_at=now-timedelta(seconds=5),expires_at=now+timedelta(seconds=295))]
...
def test_fresh_handoff_never_promotes_nonready_or_future_signal():
    now=datetime(2026,9,7,9,0,tzinfo=UTC)
    rows=[_row("armed",state="ARMED",observed_at=now-timedelta(seconds=10),expires_at=now+timedelta(seconds=290)),_row("future",observed_at=now+timedelta(seconds=5),expires_at=now+timedelta(seconds=305))]
