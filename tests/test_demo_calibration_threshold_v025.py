...
def test_demo_calibration_accepts_threshold_60(monkeypatch):
    cfg = load_project_config(None)
    monkeypatch.setenv("CTRADER_DEMO_EXECUTION_CANDIDATE_MIN", "60")

    adjusted, production = apply_demo_calibration_threshold(cfg)

    assert production == 90.0
    assert adjusted.scoring["states"]["execution_candidate_min"] == 60.0
    assert cfg.scoring["states"]["execution_candidate_min"] == 90


def test_demo_calibration_rejects_below_60(monkeypatch):
    cfg = load_project_config(None)
    monkeypatch.setenv("CTRADER_DEMO_EXECUTION_CANDIDATE_MIN", "59.99")
