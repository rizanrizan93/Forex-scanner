...
def test_runtime_profile_can_enable_stacking_but_keeps_point_zero_one_lot(monkeypatch):
...
def test_same_symbol_stack_blocks_opposite_direction():
    now = datetime.now(tz=UTC)
    block = evaluate_same_symbol_stack(row=_row(), intent=_intent(side=OrderSide.BUY, created_at=now), same_symbol_positions=(_position(side=2, opened_at=now - timedelta(minutes=10)),), symbol_info=_symbol_info(), demo_safety=_safety())
...
def test_same_symbol_stack_blocks_fourth_position():
    now = datetime.now(tz=UTC)
    positions = tuple(_position(opened_at=now - timedelta(minutes=10 + idx)) for idx in range(3))
...
def test_same_symbol_stack_blocks_aggregate_symbol_lot_cap():
    ...
    block = evaluate_same_symbol_stack(
        row=_row(),
        intent=_intent(volume=0.01, created_at=now),
        same_symbol_positions=(
            _position(lots=0.01, opened_at=now - timedelta(minutes=10)),
            _position(lots=0.01, opened_at=now - timedelta(minutes=20)),
        ),
        symbol_info=_symbol_info(),
        demo_safety=_safety(max_same_symbol_lots=0.025),
    )
