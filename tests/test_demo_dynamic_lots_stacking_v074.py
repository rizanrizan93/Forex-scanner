...
def test_runtime_profile_keeps_point_zero_one_lot_and_stacking(monkeypatch):
...
def test_high_conviction_same_direction_stack_is_allowed_after_spacing():
    ...
    block = evaluate_same_symbol_stack(
        row=_row(), intent=_intent(created_at=now),
        same_symbol_positions=(_position(side=1, opened_at=now - timedelta(minutes=10)),),
        symbol_info=_symbol_info(), demo_safety=_safety(),
    )
...
def test_same_symbol_stack_blocks_opposite_direction():
    now = datetime.now(tz=UTC)
    block = evaluate_same_symbol_stack(
        row=_row(), intent=_intent(side=OrderSide.BUY, created_at=now),
        same_symbol_positions=(_position(side=2, opened_at=now - timedelta(minutes=10)),),
        symbol_info=_symbol_info(), demo_safety=_safety(),
    )
...
def test_same_symbol_stack_blocks_below_confidence_threshold():
    now = datetime.now(tz=UTC)
    block = evaluate_same_symbol_stack(
        row=_row(score=84.99), intent=_intent(created_at=now),
        same_symbol_positions=(_position(opened_at=now - timedelta(minutes=10)),),
        symbol_info=_symbol_info(), demo_safety=_safety(),
    )
...
def test_same_symbol_stack_blocks_too_soon():
    ...
    block = evaluate_same_symbol_stack(
        row=_row(), intent=_intent(created_at=now),
        same_symbol_positions=(_position(opened_at=now - timedelta(seconds=299)),),
        symbol_info=_symbol_info(), demo_safety=_safety(),
    )
...
def test_same_symbol_stack_blocks_fourth_position():
    now = datetime.now(tz=UTC)
    positions = tuple(_position(side=1, lots=0.01, opened_at=now - timedelta(minutes=10 + idx)) for idx in range(3))
    block = evaluate_same_symbol_stack(
        row=_row(), intent=_intent(created_at=now), same_symbol_positions=positions,
        symbol_info=_symbol_info(), demo_safety=_safety(),
    )
...
def test_same_symbol_stack_blocks_aggregate_symbol_lot_cap():
    now = datetime.now(tz=UTC)
    block = evaluate_same_symbol_stack(
        row=_row(), intent=_intent(volume=0.01, created_at=now),
        ...
        symbol_info=_symbol_info(), demo_safety=_safety(max_same_symbol_lots=0.025),
    )
