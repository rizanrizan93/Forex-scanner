...
def test_continuation_tournament_is_preregistered_and_shadow_only():
    ...
    assert all(variant.bos_lookback == 12 for variant in VARIANTS)
    assert all(variant.target_r in {1.25, 1.5} for variant in VARIANTS)
    assert all(variant.adx_min in {18.0, 25.0} for variant in VARIANTS)
    assert {variant.session for variant in VARIANTS} == {"US"}
...
def test_continuation_research_workflow_is_demo_research_only():
    ...
    assert 'CTRADER_XAU_M15_RESEARCH_HISTORY_BARS: "100000"' in text
