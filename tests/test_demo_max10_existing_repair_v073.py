...
def test_signal_plan_requires_symbol_direction_and_structural_stop():
    ...
    assert row == good
    wrong_side = dict(good, direction="LONG"); store = SimpleNamespace(client=_Client([wrong_side]))
...
def test_exact_broker_identity_rejects_position_or_side_mismatch():
    trade_data=SimpleNamespace(symbolId=77,tradeSide=2,volume=100); position=SimpleNamespace(positionId=41389302,tradeData=trade_data)
    session=SimpleNamespace(reconcile=lambda:SimpleNamespace(position=[position]),symbol_id=lambda symbol:77 if symbol=="SOLUSD" else 0)
...
def test_auto_workflow_repairs_before_new_orders_and_requests_dynamic_profile():
    ...
    repair="python -m fx_scanner.demo_existing_protection_repair"; execute="python -m fx_scanner.demo_fresh_ready_handoff --limit 10"
