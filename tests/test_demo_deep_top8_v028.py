...
def test_five_core_fast_lane_keeps_non_structure_execution_guards():
    workflow = (ROOT / ".github/workflows/ctrader-demo-auto-pipeline.yml").read_text()
    ...
    assert '"STALE_SIGNAL": False' in router
    assert '"CHASE_BLOCK": False' in router
    assert '"RR_BLOCK": False' in router
    assert '"STRUCTURE_INVALID": False' in router
