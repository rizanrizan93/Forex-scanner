...
from .research_xau_opening_range_fade_v8 import (
    ARTIFACT_CONTRACT,
    RESEARCH_VERSION,
    SYMBOL,
    evaluate_opening_range_fade_v8,
)
from .storage.supabase_operational import SupabaseOperationalStore

UTC = timezone.utc
WORKER_NAME = "ctrader_xau_opening_range_fade_v8"
...
def run() -> int:
    ...
    if str(policy.ctrader.get("environment", "")).upper() != "DEMO":
        raise SystemExit("XAU_OPENING_RANGE_FADE_V8_DEMO_ONLY")
    if SYMBOL not in {pair.symbol for pair in cfg.pairs}:
        raise SystemExit("XAU_OPENING_RANGE_FADE_V8_SYMBOL_NOT_CONFIGURED")
    ...
    if len(bars) != HISTORY_BARS or not bool(spread.get("available")):
        ...
        details["decision"] = evaluate_opening_range_fade_v8(
            bars,
            costs=base_costs,
    ...
    path = Path(
        os.getenv(
            "XAU_OPENING_RANGE_FADE_V8_EVIDENCE_OUTPUT",
            "artifacts/xau-opening-range-fade-v8.json",
        )
    )
    ...
    if decision.get("stage") == "DATA_INSUFFICIENT":
        print(f"CTRADER_XAU_OR_FADE_V8 stage=DATA_INSUFFICIENT artifact={path}")
    else:
        print(
            "CTRADER_XAU_OR_FADE_V8 "
