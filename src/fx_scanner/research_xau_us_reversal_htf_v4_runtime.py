...
from .research_xau_us_reversal_htf_v4 import (
    ...
    RESEARCH_VERSION,
    SYMBOL,
    evaluate_us_reversal_htf_v4,
)
from .storage.supabase_operational import SupabaseOperationalStore

UTC = timezone.utc
WORKER_NAME = "ctrader_xau_us_reversal_htf_v4"
HISTORY_BARS = 100_000


def _artifact_path() -> Path:
    raw = os.getenv(
        "XAU_US_REVERSAL_HTF_V4_EVIDENCE_OUTPUT",
        "artifacts/xau-us-reversal-htf-v4.json",
    ).strip()
    if not raw:
        raise SystemExit("XAU_US_REVERSAL_HTF_V4_EVIDENCE_OUTPUT_REQUIRED")
    return Path(raw)
...
def run() -> int:
    ...
    if str(policy.ctrader.get("environment", "")).upper() != "DEMO":
        raise SystemExit("XAU_US_REVERSAL_HTF_V4_DEMO_ONLY")
    if not bool(policy.ctrader.get("require_demo", False)):
        raise SystemExit("XAU_US_REVERSAL_HTF_V4_REQUIRE_DEMO")
    if SYMBOL not in {pair.symbol for pair in cfg.pairs}:
        raise SystemExit("XAU_US_REVERSAL_HTF_V4_SYMBOL_NOT_CONFIGURED")
    ...
    details: dict[str, Any] = {
        ...
        "pip_size": PIP_SIZE,
        "hypothesis": (
            "V3 found US-session sweep/reversal near breakeven after stressed costs "
            "while Europe and breakout-retest were materially negative. V4 tests "
            "whether H1 EMA/HH-HL-LH-LL structure/DI context removes false reversals."
        ),
    }
    ...
    if len(bars) < MIN_HISTORY_BARS:
        ...
        print(f"CTRADER_XAU_US_REVERSAL_HTF_V4 stage=DATA_INSUFFICIENT artifact={artifact}")
        return 0
    ...
    details["decision"] = evaluate_us_reversal_htf_v4(
        bars,
        costs=base_costs,
    ...
    print(
        "CTRADER_XAU_US_REVERSAL_HTF_V4 "
