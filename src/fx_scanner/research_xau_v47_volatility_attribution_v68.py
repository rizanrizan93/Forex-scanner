...
RESEARCH_VERSION = "XAU_V47_VOLATILITY_ATTRIBUTION_V68"
ARTIFACT_CONTRACT = "XAU_V47_VOLATILITY_ATTRIBUTION_V68_EVIDENCE_1"
...
def _payload(
    ...
    *,
    ...
    for trade in trades:
        ...
        state = "UNAVAILABLE"
        if row is not None:
            candidate = str(row.get("vol_state") or "UNAVAILABLE")
            if candidate in VOL_STATES:
                state = candidate
...
def evaluate_v68(
    ...
    *,
    ...
    if not rows:
        raise ValueError("V68_EMPTY_HISTORY")
    missing = [x for x in REQUIRED_COSTS if x not in cost_scenarios]
    if missing:
        raise ValueError(f"V68_MISSING_REQUIRED_COSTS:{missing}")

    d1_context = build_secular_d1(rows)
    h1_context = build_h1_context(rows)
    vol_context = build_h1_volatility_context(rows)
    ...
    return {
        ...
        "preregistered_contract": {
            ...
            "volatility_context_contract": "V63_COMPLETED_H1_ATR14_PRIOR_120_H1_QUINTILES",
            "vol_states": list(VOL_STATES),
            "v47_family_gate_changed": False,
            "trade_filter_applied": False,
            "vol_state_selected_as_winner": False,
        ...
        "scenarios": scenarios,
        "note": (
            "V68 attributes the frozen V47 satellite by the already-frozen V63 H1 volatility "
            "state. The weak and recent windows are descriptive only; no volatility state "
            "filters or resizes trades."
        ),
    }
