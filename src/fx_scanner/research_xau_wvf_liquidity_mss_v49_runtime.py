...
from .research_xau_wvf_liquidity_mss_v49 import (
    ARTIFACT_CONTRACT,
    RESEARCH_VERSION,
    evaluate_v49,
)
...
UTC = timezone.utc
WORKER_NAME = "dukascopy_xau_wvf_liquidity_mss_v49"


def run() -> int:
    era_id = os.environ.get("V49_ERA", "").strip()
    if era_id not in ERAS:
        raise SystemExit(f"V49_ERA_INVALID:{era_id}")
    era = ERAS[era_id]
    bars = _fetch(era)

    decision = evaluate_v49(
        bars,
        era_id=era_id,
        era_start=era["start"],
        era_end=era["end"],
    ...
    path = Path(
        os.getenv(
            "V49_EVIDENCE_OUTPUT",
            f"artifacts/xau-wvf-liquidity-mss-v49-{era_id}.json",
        )
    )
    ...
    print(
        f"V49_RESULT era={era_id} rows={len(bars)} artifact={path} "
    ...
    for cost_id, scenario in decision["scenario_results"].items():
        for variant, payload in scenario["variants"].items():
            print(
                f"V49_VARIANT era={era_id} cost={cost_id} id={variant} "
