...
from .research_xau_asymmetric_regime_family_v44 import (
    ARTIFACT_CONTRACT,
    RESEARCH_VERSION,
    evaluate_v44,
)
...
UTC = timezone.utc
WORKER_NAME = "dukascopy_xau_asymmetric_regime_family_v44"


def run() -> int:
    era_id = os.environ.get("V44_ERA", "").strip()
    if era_id not in ERAS:
        raise SystemExit(f"V44_ERA_INVALID:{era_id}")
    era = ERAS[era_id]
    bars = _fetch(era)

    decision = evaluate_v44(
        bars,
        era_id=era_id,
        era_start=era["start"],
        era_end=era["end"],
    ...
    path = Path(
        os.getenv(
            "V44_EVIDENCE_OUTPUT",
            f"artifacts/xau-asymmetric-regime-family-v44-{era_id}.json",
        )
    )
    ...
    print(
        f"V44_RESULT era={era_id} rows={len(bars)} artifact={path} "
    ...
    for cost_id, scenario in decision["scenario_results"].items():
        for portfolio_id, payload in scenario["portfolios"].items():
            print(
                f"V44_PORTFOLIO era={era_id} cost={cost_id} id={portfolio_id} "
        ...
        if cost_id == "V24_STRESS_4675":
            for route, payload in scenario["routes"].items():
                print(
                    f"V44_ROUTE era={era_id} id={route} "
                    f"{_metric_line(payload['metrics'])} "
                    f"tpd={payload['trades_per_day']} "
                    f"loss_streak={payload['max_losing_streak']}"
                )
            print(f"V44_COMPONENTS era={era_id} {scenario['component_counts']}")
    return 0
