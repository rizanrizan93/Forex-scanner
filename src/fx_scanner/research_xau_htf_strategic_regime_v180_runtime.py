...
SYMBOL = "XAUUSD"
WORKER_NAME = "ctrader_xau_htf_strategic_regime_v180"


def _artifact_path() -> Path:
    raw = os.getenv(
        "XAU_HTF_REGIME_V180_OUTPUT",
        "artifacts/xau-htf-strategic-regime-v180.json",
    ).strip()
    if not raw:
        raise SystemExit("XAU_HTF_REGIME_V180_OUTPUT_REQUIRED")
    return Path(raw)
...
def run() -> int:
    ...
    if str(policy.ctrader.get("environment", "")).upper() != "DEMO":
        raise SystemExit("XAU_HTF_REGIME_V180_DEMO_ONLY")
    if not bool(policy.ctrader.get("require_demo", False)):
        raise SystemExit("XAU_HTF_REGIME_V180_REQUIRE_DEMO")
    if SYMBOL not in cfg.pair_map:
        raise SystemExit("XAU_HTF_REGIME_V180_SYMBOL_NOT_CONFIGURED")
    ...
    details["evaluation"] = evaluate_htf_regime_research(bars)
    details["decision"] = details["evaluation"]["decision"]
    artifact = _write_artifact(details)
    healthy = details["decision"] != "DATA_INSUFFICIENT"
    _heartbeat(details, healthy=healthy)

    current = dict(details["evaluation"].get("current") or {})
    pool = dict(details["evaluation"].get("zone_pool") or {})
    print(
        "XAU_HTF_REGIME_V180 "
        f"bars={len(bars)} bias={current.get('strategic_bias')} "
        f"score={current.get('raw_score')} "
        f"canonical_zones={pool.get('canonical_count')} "
        f"shadow_24_48h={pool.get('shadow_count')} "
