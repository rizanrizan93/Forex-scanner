...
from .research_xau_turtle_integration_v117 import ARTIFACT_CONTRACT,RESEARCH_VERSION,evaluate_v117
...
UTC=timezone.utc
WORKER_NAME="dukascopy_xau_turtle_integration_v117"
...
def run()->int:
    ...
    p=Path(os.getenv("V117_EVIDENCE_OUTPUT","artifacts/xau-turtle-integration-v117.json"))
    ...
    for cost,x in d["scenarios"].items():
        for name,m in x["full"].items():
            print(f"V117_FULL cost={cost} id={name} {_metric_line(m)}")
        for era,v in x["eras"].items():
            for name,m in v.items():
                print(f"V117_ERA cost={cost} era={era} id={name} {_metric_line(m)}")
        for sy,v in x["start_sensitivity"].items():
            for name,m in v.items():
                print(f"V117_START cost={cost} start={sy} id={name} {_metric_line(m)}")
        for name,c in x["cash_fixed_001_no_risk_cap"].items():
            print(f"V117_CASH cost={cost} id={name} ending={c['ending_balance_usd']} return_pct={c['return_pct']} opened={c['opened_trades']} margin_skips={c['margin_skips']} guard_skips={c['stopout_guard_skips']} dd_pct={c['max_realized_drawdown_pct']} ruin={c['ruin']} hit1000={c['hit_1000']} hit1000_at={c['hit_1000_at']}")
    return 0
