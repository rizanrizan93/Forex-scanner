...
from .research_xau_expansion_species_v99 import ARTIFACT_CONTRACT,RESEARCH_VERSION,evaluate_v99
...
UTC=timezone.utc
WORKER_NAME="dukascopy_xau_expansion_species_v99"
...
def run()->int:
    ...
    p=Path(os.getenv("V99_EVIDENCE_OUTPUT","artifacts/xau-expansion-species-v99.json"));p.parent.mkdir(parents=True,exist_ok=True)
    ...
    for k,m in d["full_period"].items(): print(f"V99_FULL id={k} {_metric_line(m)}")
    for era,x in d["eras"].items():
        print("V99_SPECIES era="+era+" "+json.dumps(d["species_counts_by_era"][era],sort_keys=True))
        print("V99_FIRST era="+era+" "+json.dumps(d["first_species_date_by_era"][era],sort_keys=True))
        for k in ("core","selector"):
            print(f"V99_ERA id={k} era={era} {_metric_line(x[k])}")
            c=x["cash_fresh_100"][k]
            print(f"V99_CASH id={k} era={era} ending={c['ending_balance_usd']} return_pct={c['return_pct']} opened={c['opened_trades']} ruin={c['ruin']}")
    for k,g in d["satellite_gates"].items(): print("V99_GATE id="+k+" "+json.dumps(g,sort_keys=True,default=str))
    return 0
