from __future__ import annotations

from datetime import datetime, timezone
from typing import Any, Sequence

import numpy as np
import pandas as pd

from .demo_donchian_adaptive_tournament import TournamentTrade, compute_metrics
from .models import Bar, ensure_utc
from .research_xau_2025_bootstrap_onset_forensic_v122 import build_v122_feature_frame
from .research_xau_champion_onset_reaccel_v123 import build_reaccel_route_frame
from .research_xau_false_onset_forensic_v125 import annotate_compressed_epochs
from .research_xau_multihorizon_100usd_v20 import _portfolio_candidates
from .research_xau_qualitative_reaccel_gate_v126 import passes_qualitative_gate
from .research_xau_runtime_feasibility_v128 import (
    LOT,
    MAX_POSITIONS,
    PORTFOLIO_RISK_PCT,
    STOPOUT_PCT,
    _loss_to_stop,
    _margin_for_trade,
)
from .research_xau_v110_robustness_v111 import assemble_v110_selector
from .research_xau_v126_robustness_v127 import _route

RESEARCH_VERSION="XAU_CAPITAL_COMPATIBILITY_V129"
ARTIFACT_CONTRACT="XAU_CAPITAL_COMPATIBILITY_V129_EVIDENCE_1"
POLICY_EFFECT="SHADOW_ONLY"
EXECUTION_INFLUENCE=False
PROMOTION_ELIGIBLE=False

PER_TRADE_RISK_PCT=20.0
MARGIN_CAPS=(25.0,50.0,75.0)
ACCOUNT_SCENARIOS=(
    ("LIVE_100_1_100",100.0,100.0),
    ("DEMO_342_1_30",342.0,30.0),
)
WINDOWS=(
    ("FULL_2012_2026",datetime(2012,1,1,tzinfo=timezone.utc)),
    ("RECENT_2025_2026",datetime(2025,1,1,tzinfo=timezone.utc)),
)


def _metrics(trades:Sequence[TournamentTrade])->dict[str,Any]:
    return compute_metrics(tuple(trades)).payload()


def _period(trades:Sequence[TournamentTrade],start,end)->tuple[TournamentTrade,...]:
    a=ensure_utc(start);b=ensure_utc(end)
    return tuple(t for t in trades if a<=ensure_utc(t.entry_at)<b)


def _capital_floor(
    trade:TournamentTrade,
    *,
    spec,
    tiers,
    account_leverage:float,
    margin_cap_pct:float,
)->float:
    loss=_loss_to_stop(trade,spec=spec)
    margin=_margin_for_trade(trade,spec=spec,tiers=tiers,account_leverage=account_leverage)
    risk_floor=loss/(PER_TRADE_RISK_PCT/100.0)
    margin_floor=margin/(float(margin_cap_pct)/100.0)
    return float(max(risk_floor,margin_floor))


def _floor_distribution(
    trades:Sequence[TournamentTrade],
    *,
    spec,
    tiers,
    account_leverage:float,
    margin_cap_pct:float,
)->dict[str,Any]:
    vals=np.asarray([
        _capital_floor(
            t,spec=spec,tiers=tiers,account_leverage=account_leverage,
            margin_cap_pct=margin_cap_pct,
        )
        for t in trades
    ],dtype=float)
    if len(vals)==0:
        return {"n":0}
    return {
        "n":int(len(vals)),
        "min":float(vals.min()),
        "p10":float(np.quantile(vals,.10)),
        "p25":float(np.quantile(vals,.25)),
        "median":float(np.median(vals)),
        "p75":float(np.quantile(vals,.75)),
        "p90":float(np.quantile(vals,.90)),
        "max":float(vals.max()),
        "pct_feasible_at_100":float(np.mean(vals<=100.0)),
        "pct_feasible_at_150":float(np.mean(vals<=150.0)),
        "pct_feasible_at_200":float(np.mean(vals<=200.0)),
        "pct_feasible_at_342":float(np.mean(vals<=342.0)),
    }


def _simulate(
    trades:Sequence[TournamentTrade],
    *,
    spec,
    tiers,
    starting_balance:float,
    account_leverage:float,
    margin_cap_pct:float,
)->dict[str,Any]:
    balance=float(starting_balance)
    peak=balance
    min_balance=balance
    max_dd_pct=0.0
    active:dict[int,dict[str,float]]={}
    accepted:list[TournamentTrade]=[]
    risk_skips=portfolio_skips=margin_skips=stopout_skips=maxpos_skips=0
    events=[]
    ordered=sorted(trades,key=lambda t:(ensure_utc(t.entry_at),ensure_utc(t.exit_at)))
    for idx,t in enumerate(ordered):
        events.append((ensure_utc(t.entry_at),1,idx,"ENTRY",t))
        events.append((ensure_utc(t.exit_at),0,idx,"EXIT",t))
    events.sort(key=lambda x:(x[0],x[1],x[2]))

    for _,_,idx,kind,trade in events:
        if kind=="EXIT":
            state=active.pop(idx,None)
            if state is None:
                continue
            balance+=state["pnl_usd"]
            peak=max(peak,balance)
            min_balance=min(min_balance,balance)
            if peak>0:
                max_dd_pct=max(max_dd_pct,100.0*(peak-balance)/peak)
            continue

        if balance<=0:
            continue
        if len(active)>=MAX_POSITIONS:
            maxpos_skips+=1
            continue

        loss=_loss_to_stop(trade,spec=spec)
        if loss>balance*(PER_TRADE_RISK_PCT/100.0)+1e-9:
            risk_skips+=1
            continue

        existing_risk=sum(x["planned_loss_usd"] for x in active.values())
        if existing_risk+loss>balance*(PORTFOLIO_RISK_PCT/100.0)+1e-9:
            portfolio_skips+=1
            continue

        current_margin=sum(x["margin_usd"] for x in active.values())
        margin_free=max(0.0,balance-current_margin)
        margin=_margin_for_trade(trade,spec=spec,tiers=tiers,account_leverage=account_leverage)
        if margin>margin_free*(float(margin_cap_pct)/100.0)+1e-9:
            margin_skips+=1
            continue

        candidate_margin=current_margin+margin
        worst_equity=balance-(existing_risk+loss)
        margin_level=float("inf") if candidate_margin<=0 else 100.0*worst_equity/candidate_margin
        if margin_level<=STOPOUT_PCT:
            stopout_skips+=1
            continue

        risk_price=abs(float(trade.entry_price)-float(trade.stop_loss))
        units=float(spec.contract_units_per_lot)*LOT
        pnl=float(trade.net_r)*risk_price*units
        active[idx]={"planned_loss_usd":loss,"margin_usd":margin,"pnl_usd":pnl}
        accepted.append(trade)

    m=_metrics(accepted)
    return {
        "available":len(trades),
        "opened":len(accepted),
        "acceptance_rate":0.0 if not trades else float(len(accepted)/len(trades)),
        "ending_balance_usd":float(balance),
        "return_pct":float((balance/starting_balance-1.0)*100.0),
        "minimum_balance_usd":float(min_balance),
        "max_realized_drawdown_pct":float(max_dd_pct),
        "risk_skips":risk_skips,
        "portfolio_risk_skips":portfolio_skips,
        "margin_skips":margin_skips,
        "stopout_skips":stopout_skips,
        "max_positions_skips":maxpos_skips,
        "accepted_metrics":m,
    }


def _component_breakdown(
    trades:Sequence[TournamentTrade],
    *,
    end,
    spec,
    tiers,
)->dict[str,Any]:
    ids=sorted({str(t.strategy_id) for t in trades})
    out={}
    for sid in ids:
        component=tuple(t for t in trades if str(t.strategy_id)==sid)
        windows={}
        for wid,start in WINDOWS:
            vals=_period(component,start,end)
            scenarios={}
            for aid,balance,lev in ACCOUNT_SCENARIOS:
                cap_rows={}
                for margin_cap in MARGIN_CAPS:
                    cap_rows[str(int(margin_cap))]={
                        "floor_distribution":_floor_distribution(
                            vals,spec=spec,tiers=tiers,account_leverage=lev,
                            margin_cap_pct=margin_cap,
                        ),
                        "simulation":_simulate(
                            vals,spec=spec,tiers=tiers,starting_balance=balance,
                            account_leverage=lev,margin_cap_pct=margin_cap,
                        ),
                    }
                scenarios[aid]=cap_rows
            windows[wid]={
                "unconstrained_metrics":_metrics(vals),
                "scenarios":scenarios,
            }
        out[sid]=windows
    return out


def evaluate_v129(
    rows:Sequence[Bar],
    *,
    evaluation_end,
    pip_size:float,
    costs,
    broker_spec,
    leverage_tiers,
)->dict[str,Any]:
    bars=tuple(sorted(rows,key=lambda x:ensure_utc(x.timestamp)))
    end=ensure_utc(evaluation_end)

    v24=_portfolio_candidates(
        bars,base_costs=costs,stressed_costs=costs,pip_size=pip_size
    )["D1_PLUS_L12_L20"]["stress"]
    v24=_period(v24,datetime(2012,1,1,tzinfo=timezone.utc),end)

    frame=build_v122_feature_frame(bars)
    _,epoch_defs=build_reaccel_route_frame(frame,route_id="COMPRESSED_REACCEL")
    _,selector_all,_=assemble_v110_selector(
        bars,costs=costs,pip_size=pip_size,evaluation_end=end
    )
    selector=tuple(t for t in selector_all if ensure_utc(t.entry_at)<end)
    epoch_records=annotate_compressed_epochs(frame,epoch_defs,selector)
    v126=_route(selector,epoch_records,end=end,predicate=passes_qualitative_gate)
    v126=_period(v126,datetime(2012,1,1,tzinfo=timezone.utc),end)

    portfolios={
        "V24_EXACT":v24,
        "V126_GATED_V110":v126,
    }
    result={}
    for pid,trades in portfolios.items():
        result[pid]={
            "full_unconstrained_metrics":_metrics(trades),
            "components":_component_breakdown(
                trades,end=end,spec=broker_spec,tiers=leverage_tiers,
            ),
        }

    # Current-price feasibility boundary for one 0.01 XAU order, independent of setup SL.
    current_price=4370.0
    units=float(broker_spec.contract_units_per_lot)*LOT
    live_margin=current_price*units/100.0
    demo_margin=current_price*units/30.0
    current_margin_boundaries={
        "LIVE_1_100":{
            str(int(cap)):float(live_margin/(cap/100.0))
            for cap in MARGIN_CAPS
        },
        "DEMO_1_30":{
            str(int(cap)):float(demo_margin/(cap/100.0))
            for cap in MARGIN_CAPS
        },
    }

    return {
        "research_version":RESEARCH_VERSION,
        "artifact_contract":ARTIFACT_CONTRACT,
        "policy_effect":POLICY_EFFECT,
        "execution_influence":EXECUTION_INFLUENCE,
        "promotion_eligible":PROMOTION_ELIGIBLE,
        "live_execution_enabled":False,
        "contract":{
            "purpose":"diagnose capital compatibility by strategy component and margin cap",
            "lot":LOT,
            "per_trade_risk_pct":PER_TRADE_RISK_PCT,
            "aggregate_risk_pct":PORTFOLIO_RISK_PCT,
            "margin_caps_pct":list(MARGIN_CAPS),
            "margin_caps_are_diagnostic_only":True,
            "current_production_margin_cap_pct":25.0,
            "max_positions":MAX_POSITIONS,
            "stopout_pct":STOPOUT_PCT,
            "no_strategy_rule_changes":True,
            "no_selection_or_promotion":True,
            "no_execution_authority":True,
        },
        "current_price_margin_only_min_balance":current_margin_boundaries,
        "portfolios":result,
    }
