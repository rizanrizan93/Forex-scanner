...
from .research_fx_cross_sectional_strength_v28 import (
    ARTIFACT_CONTRACT, FX_SYMBOLS, RESEARCH_VERSION, evaluate_v28,
)
...
UTC = timezone.utc
WORKER_NAME = "ctrader_fx_cross_sectional_strength_v28"
    ...
    for delay in CONNECT_BACKOFF:
        ...
        try:
            ...
            if "connection timeout" not in str(exc).lower():
                raise
    raise CollectorUnavailable("V28_CTRADER_CONNECT_RETRY_EXHAUSTED") from last
...
def run() -> int:
    ...
    if str(policy.ctrader.get("environment", "")).upper() != "DEMO":
        raise SystemExit("V28_DEMO_ONLY")
    if not bool(policy.ctrader.get("require_demo", False)):
        raise SystemExit("V28_REQUIRE_DEMO")

    pair_cfg = {p.symbol:p for p in cfg.pairs}
    if any(s not in pair_cfg for s in FX_SYMBOLS):
        raise SystemExit("V28_PAIR_CONFIG_MISMATCH")
    ...
    try:
        for symbol in FX_SYMBOLS:
            try:
                ...
                evidence[symbol] = {"bars":0,"error":str(exc)}
                continue
            ...
            pip_sizes[symbol] = float(pair.pip_size)
            base_costs[symbol] = bc
            stressed_costs[symbol] = sc
    finally:
        try: feed.close()
    ...
    if len(datasets) < 13:
        details["decision"] = {
            ...
            "usable_symbols":sorted(datasets),
            "promotion_eligible":False,
        }
    else:
        details["decision"] = evaluate_v28(
            datasets, pip_sizes=pip_sizes,
            base_costs=base_costs, stressed_costs=stressed_costs,
        )
    ...
    path=Path(os.getenv("V28_EVIDENCE_OUTPUT","artifacts/fx-cross-sectional-strength-v28.json"))
    ...
    if d.get("stage")=="DATA_INSUFFICIENT":
        print(f"V28_RESULT stage=DATA_INSUFFICIENT usable={d['usable_symbols']} artifact={path}")
    else:
        print(
            "V28_RESULT "
