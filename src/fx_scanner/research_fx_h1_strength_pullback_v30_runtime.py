...
from .research_fx_h1_strength_pullback_v30 import ARTIFACT_CONTRACT,FX_SYMBOLS,RESEARCH_VERSION,evaluate_v30
...
UTC=timezone.utc
WORKER_NAME="ctrader_fx_h1_strength_pullback_v30"
...
def _feed(policy):
    ...
    for delay in CONNECT_BACKOFF:
        ...
        try:
            feed=build_ctrader_research_feed(policy,FX_SYMBOLS); feed.ensure_connected(); return feed
        except CollectorUnavailable as exc:
            last=exc
            if "connection timeout" not in str(exc).lower(): raise
    raise CollectorUnavailable("V30_CTRADER_CONNECT_RETRY_EXHAUSTED") from last
...
def _fetch(feed,symbol,as_of):
    ...
    for page in range(1,MAX_PAGES+1):
        ...
        pages.append({"page":page,"received":len(got),"merged":len(merged),
                      "earliest":earliest.isoformat(),"latest":max(x.timestamp for x in got).isoformat()})
...
def run()->int:
    ...
    if str(policy.ctrader.get("environment","")).upper()!="DEMO": raise SystemExit("V30_DEMO_ONLY")
    if not bool(policy.ctrader.get("require_demo",False)): raise SystemExit("V30_REQUIRE_DEMO")
    ...
    try:
        for symbol in FX_SYMBOLS:
            try: bars,pages=_fetch(feed,symbol,now)
            except CollectorUnavailable as exc:
                evidence[symbol]={"bars":0,"error":str(exc)}; continue
            pair=pair_cfg[symbol]
            spread=infer_spread_proxy_pips(bars,pip_size=float(pair.pip_size)) if bars else {"available":False}
    ...
    details={"research_version":RESEARCH_VERSION,"environment":"DEMO","policy_effect":"SHADOW_ONLY",
             "execution_influence":False,"observed_at":now.isoformat(),
             "history_target_per_symbol":HISTORY_TARGET,"data_evidence":evidence}
    if len(datasets)<13:
        details["decision"]={"stage":"DATA_INSUFFICIENT","usable_symbols":sorted(datasets),"promotion_eligible":False}
    else:
        details["decision"]=evaluate_v30(datasets,pip_sizes=pip_sizes,base_costs=base_costs,stressed_costs=stressed_costs)
    try:
        SupabaseOperationalStore.from_env().write_heartbeat(WORKER_NAME,healthy=True,lag_seconds=0.0,details=details)
    except Exception as exc:
        details["heartbeat_write_error"]=f"{type(exc).__name__}:{exc}"

    path=Path(os.getenv("V30_EVIDENCE_OUTPUT","artifacts/fx-h1-strength-pullback-v30.json"))
    ...
    if d.get("stage")=="DATA_INSUFFICIENT":
        print(f"V30_RESULT stage=DATA_INSUFFICIENT usable={d['usable_symbols']} artifact={path}")
    else:
        print(
            "V30_RESULT "
