...
from .research_xau_opening_range_breakout_v10 import (
    ARTIFACT_CONTRACT,
    RESEARCH_VERSION,
    SYMBOL,
    evaluate_opening_range_breakout_v10,
)
from .storage.supabase_operational import SupabaseOperationalStore

UTC = timezone.utc
WORKER_NAME = "ctrader_xau_opening_range_breakout_v10"
...
def run() -> int:
    ...
    if str(policy.ctrader.get("environment", "")).upper() != "DEMO":
        raise SystemExit("XAU_OR_BREAKOUT_V10_DEMO_ONLY")
    if SYMBOL not in {pair.symbol for pair in cfg.pairs}:
        raise SystemExit("XAU_OR_BREAKOUT_V10_SYMBOL_NOT_CONFIGURED")
    ...
    if len(bars) != HISTORY_BARS or not bool(spread.get("available")):
        ...
        details["decision"] = evaluate_opening_range_breakout_v10(
            bars,
            costs=base_costs,
    ...
    path = Path(
        os.getenv(
            "XAU_OR_BREAKOUT_V10_EVIDENCE_OUTPUT",
            "artifacts/xau-opening-range-breakout-v10.json",
        )
    )
    ...
    if decision.get("stage") == "DATA_INSUFFICIENT":
        print(f"CTRADER_XAU_OR_BREAKOUT_V10 stage=DATA_INSUFFICIENT artifact={path}")
    else:
        print(
            "CTRADER_XAU_OR_BREAKOUT_V10 "
