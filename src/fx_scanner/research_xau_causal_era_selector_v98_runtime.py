...
from .research_xau_causal_era_selector_v98 import ARTIFACT_CONTRACT,RESEARCH_VERSION,evaluate_v98
...
UTC=timezone.utc
WORKER_NAME="dukascopy_xau_causal_era_selector_v98"
...
def run()->int:
    ...
    p=Path(os.getenv("V98_EVIDENCE_OUTPUT","artifacts/xau-causal-era-selector-v98.json"));p.parent.mkdir(parents=True,exist_ok=True)
    ...
    for k,m in d["full_period"].items(): print(f"V98_FULL id={k} {_metric_line(m)}")
    for era,x in d["eras"].items():
        print("V98_STATE era="+era+" "+json.dumps(d["state_counts_by_era"][era],sort_keys=True))
        for k in ("core","selector","static_all_aggressive"):
            print(f"V98_ERA id={k} era={era} {_metric_line(x[k])}")
            c=x["cash_fresh_100"][k]
            print(f"V98_CASH id={k} era={era} ending={c['ending_balance_usd']} return_pct={c['return_pct']} opened={c['opened_trades']} guard_skips={c.get('planned_stop_guard_skips')} margin_skips={c.get('margin_skips')} ruin={c['ruin']}")
    for k,g in d["satellite_gates"].items(): print("V98_GATE id="+k+" "+json.dumps(g,sort_keys=True,default=str))
    return 0
