...
UTC = timezone.utc
WORKER_NAME = "dukascopy_xau_external_liquidity_cascade_v51"


def run() -> int:
    era_id = os.environ.get("V51_ERA", "").strip()
    if era_id not in ERAS:
        raise SystemExit(f"V51_ERA_INVALID:{era_id}")
    era = ERAS[era_id]
    bars = _fetch(era)
    decision = evaluate_v51(
        bars,
        era_id=era_id,
        era_start=era["start"],
        era_end=era["end"],
    ...
    path = Path(
        os.getenv(
            "V51_EVIDENCE_OUTPUT",
            f"artifacts/xau-external-liquidity-cascade-v51-{era_id}.json",
        )
    )
    ...
    print(
        f"V51_RESULT era={era_id} rows={len(bars)} "
        f"raw_cascades={decision['raw_cascade_signals']} "
    ...
    for cost_id, scenario in decision["scenario_results"].items():
        for variant, payload in scenario["variants"].items():
            print(
                f"V51_VARIANT era={era_id} cost={cost_id} id={variant} "
