...
class CTraderResearchFeed:
    ...
    def quote(self, symbol: str, *, at: datetime | None = None):
        """Return a quote only when broker schedule is open.

        ``at`` exists solely for deterministic schedule testing/research. Runtime
        callers omit it and continue to use the current broker clock.
        """
