...
from .research_xau_v47_target_credibility_v74 import (
    ARTIFACT_CONTRACT,
    FULL_END,
    FULL_START,
    RESEARCH_VERSION,
...
UTC = timezone.utc
WORKER_NAME = "dukascopy_xau_v47_target_credibility_v74"
FULL_ERA = {
...
def run() -> int:
    ...
    path = Path(os.getenv("V74_EVIDENCE_OUTPUT", "artifacts/xau-v47-target-credibility-v74.json"))
    ...
    print(
        f"V74_RESULT rows={len(bars)} artifact={path} "
    ...
    for cost_id, payload in decision["scenarios"].items():
        ...
        for window, row in payload["diagnostic_windows"].items():
            ...
            for bucket, stats in row["stop_to_h1_atr"].items():
                if int(stats["trades"]) > 0:
                    print(
                        f"V74_STOP cost={cost_id} window={window} bucket={bucket} "
                        f"{_metric_line(stats['metrics'])}"
                    )
            for bucket, stats in row["target_to_d1_median60"].items():
                if int(stats["trades"]) > 0:
                    print(
                        f"V74_TARGET cost={cost_id} window={window} bucket={bucket} "
                        f"{_metric_line(stats['metrics'])}"
                    )
    return 0
