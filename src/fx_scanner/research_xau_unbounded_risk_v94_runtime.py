...
from .research_xau_unbounded_risk_v94 import ARTIFACT_CONTRACT,RESEARCH_VERSION,evaluate_v94
...
UTC=timezone.utc
WORKER_NAME="dukascopy_xau_unbounded_risk_v94"
...
def run()->int:
    ...
    p=Path(os.getenv("V94_EVIDENCE_OUTPUT","artifacts/xau-unbounded-risk-v94.json"));p.parent.mkdir(parents=True,exist_ok=True)
    ...
    for vid,modes in d["variants"].items():
      for mode,c in modes.items():
        print(f"V94_CASH id={vid} mode={mode} ending={c['ending_balance_usd']} return_pct={c['return_pct']} min_balance={c['minimum_realized_balance_usd']} max_dd_pct={c['max_realized_drawdown_pct']} opened={c['opened_trades']} margin_skips={c['margin_skips']} max_lot_mode={mode} hit1000={c['hit_1000']} hit1000_at={c['hit_1000_at']} ruin={c['ruin']} worst_stop_equity={c['minimum_worst_case_equity_at_planned_stops_usd']} below_zero_stop_events={c['planned_stop_equity_below_zero_events']}")
    return 0
