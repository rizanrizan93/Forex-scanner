...
from .research_xau_canonical_gate_v4 import (
    ARTIFACT_CONTRACT,
    RESEARCH_VERSION,
    SYMBOL,
    evaluate_canonical_gate_v4,
)
...
UTC = timezone.utc
WORKER_NAME = "ctrader_xau_canonical_gate_v4"
HISTORY_BARS = 100_000


def _artifact_path() -> Path:
    raw = os.getenv(
        "XAU_CANONICAL_GATE_V4_EVIDENCE_OUTPUT",
        "artifacts/xau-canonical-gate-v4.json",
    ).strip()
    if not raw:
        raise SystemExit("XAU_CANONICAL_GATE_V4_EVIDENCE_OUTPUT_REQUIRED")
    return Path(raw)
...
def run() -> int:
    ...
    if str(policy.ctrader.get("environment", "")).upper() != "DEMO":
        raise SystemExit("XAU_CANONICAL_GATE_V4_DEMO_ONLY")
    if not bool(policy.ctrader.get("require_demo", False)):
        raise SystemExit("XAU_CANONICAL_GATE_V4_REQUIRE_DEMO")
    if SYMBOL not in {pair.symbol for pair in cfg.pairs}:
        raise SystemExit("XAU_CANONICAL_GATE_V4_SYMBOL_NOT_CONFIGURED")
    ...
    details: dict[str, Any] = {
        ...
        "pip_size": PIP_SIZE,
        "hypothesis": (
            "The current EMA-SMC-ICT canonical authority may be suppressing XAU "
            "opportunity frequency through one or more stacked gates. V4 replays "
            "the production evidence functions and preregistered gate relaxations "
            "to measure frequency versus net expectancy without changing execution."
        ),
    }
    ...
    if len(bars) < MIN_HISTORY_BARS:
        ...
        print(f"CTRADER_XAU_CANONICAL_GATE_V4 stage=DATA_INSUFFICIENT artifact={artifact}")
        return 0
    ...
    details["decision"] = evaluate_canonical_gate_v4(
        bars,
        costs=base_costs,
    ...
    print(
        "CTRADER_XAU_CANONICAL_GATE_V4 "
        f"bars={len(bars)}/{HISTORY_BARS} spread_pips={float(spread['median_pips']):.4f} "
        f"prefilter={diagnostics['prefilter_candidate_bars']} "
        f"exact_evals={diagnostics['exact_model_evaluations']} "
