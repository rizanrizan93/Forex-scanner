...
from .research_xau_short_confirmatory_v7 import (
    ARTIFACT_CONTRACT,
    RESEARCH_VERSION,
    SYMBOL,
    evaluate_short_confirmatory_v7,
)
from .storage.supabase_operational import SupabaseOperationalStore

UTC = timezone.utc
WORKER_NAME = "ctrader_xau_short_confirmatory_v7"
...
def run() -> int:
    ...
    if str(policy.ctrader.get("environment", "")).upper() != "DEMO":
        raise SystemExit("XAU_SHORT_CONFIRMATORY_V7_DEMO_ONLY")
    if SYMBOL not in {pair.symbol for pair in cfg.pairs}:
        raise SystemExit("XAU_SHORT_CONFIRMATORY_V7_SYMBOL_NOT_CONFIGURED")
    ...
    if len(bars) != HISTORY_BARS or not bool(spread.get("available")):
        ...
        details["decision"] = evaluate_short_confirmatory_v7(
            bars,
            costs=base_costs,
    ...
    path = Path(
        os.getenv(
            "XAU_SHORT_CONFIRMATORY_V7_EVIDENCE_OUTPUT",
            "artifacts/xau-short-confirmatory-v7.json",
        )
    )
    ...
    if decision.get("stage") == "DATA_INSUFFICIENT":
        print(
            "CTRADER_XAU_SHORT_CONFIRMATORY_V7 "
            f"stage=DATA_INSUFFICIENT reason={decision['reason']} artifact={path}"
        )
    else:
        holdout = decision["holdout"]["base"]
        print(
            "CTRADER_XAU_SHORT_CONFIRMATORY_V7 "
            f"holdout_trades={holdout['completed_trades']} "
            f"win_rate={holdout['win_rate']} pf={holdout['profit_factor']} "
            f"expectancy_r={holdout['expectancy_r']} "
