...
def run():
    ...
    p=Path(os.getenv("V152_EVIDENCE_OUTPUT","artifacts/xau-afic-liquidity-forecast-v152.json"));p.parent.mkdir(parents=True,exist_ok=True);p.write_text(json.dumps(d,indent=2,sort_keys=True,default=str)+"\n")
    for v,x in d["full"].items():print("V152_FULL "+json.dumps({"variant":v,**_c(x)},sort_keys=True))
    for era,b in d["eras"].items():
        for v,x in b.items():print("V152_ERA "+json.dumps({"era":era,"variant":v,**_c(x)},sort_keys=True))
    print("V152_DECISION "+json.dumps({"research_only":True,"production_promotion":False},sort_keys=True))
    return 0
