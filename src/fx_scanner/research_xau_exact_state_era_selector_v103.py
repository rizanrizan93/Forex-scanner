...
RESEARCH_VERSION="XAU_EXACT_STATE_ERA_SELECTOR_V103"
ARTIFACT_CONTRACT="XAU_EXACT_STATE_ERA_SELECTOR_V103_EVIDENCE_1"
...
def gate_exact_state_only(
    ...
    return tuple(kept),{
        ...
        "hierarchy":["EXACT_RECENT_126D","EXACT_POST_CHANGEPOINT_EPOCH","OFF_IF_INSUFFICIENT"],
...
def evaluate_v103(
    ...
    for name in SATELLITES:
        selected[name],gates[name]=gate_exact_state_only(
    ...
    return {
        ...
        "contract":{
            ...
            "exact_post_changepoint_epoch_available":True,
            "broad_fallback_allowed":False,
            "health_thresholds_unchanged":True,
            "calendar_year_used":False,
            "threshold_grid_search":False,
            "exploratory_after_v102":True,
        },
        "change_points":list(change_points),
        "strategy_state_gates":gates,
        "full_period":{"core":_metrics(core),"selector":_metrics(selector)},
        "eras":era_out,
        "note":"V103 removes all cross-species broad fallback. Every aggressive strategy-state pair must prove exact-species health using either the recent 126D window or the full post-change-point epoch, with frozen thresholds.",
    }
