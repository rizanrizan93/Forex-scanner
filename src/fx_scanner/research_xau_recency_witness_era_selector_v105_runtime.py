...
from .research_xau_recency_witness_era_selector_v105 import ARTIFACT_CONTRACT,RESEARCH_VERSION,evaluate_v105
...
UTC=timezone.utc
WORKER_NAME="dukascopy_xau_recency_witness_era_selector_v105"
...
def run()->int:
    ...
    p=Path(os.getenv("V105_EVIDENCE_OUTPUT","artifacts/xau-recency-witness-era-selector-v105.json"));p.parent.mkdir(parents=True,exist_ok=True)
    ...
    for k,m in d["full_period"].items(): print(f"V105_FULL id={k} {_metric_line(m)}")
    for era,x in d["eras"].items():
        for k in ("core","selector"):
            print(f"V105_ERA id={k} era={era} {_metric_line(x[k])}")
            c=x["cash_fresh_100"][k]
            print(f"V105_CASH id={k} era={era} ending={c['ending_balance_usd']} return_pct={c['return_pct']} opened={c['opened_trades']} ruin={c['ruin']}")
        for s,m in x["selected_strategy_contribution"].items():
            print(f"V105_CONTRIB strategy={s} era={era} {_metric_line(m)}")
    for k,g in d["strategy_state_gates"].items(): print("V105_GATE id="+k+" "+json.dumps(g,sort_keys=True,default=str))
    return 0
