...
def load_demo_execution_policy(root=None) -> ExecutionPolicy:
    ...
    The Auto workflow may opt into up to ten total positions while every order
    remains capped at 0.01 lot. Same-symbol stacking remains available only
    behind the independent qualification gate. LIVE remains untouched.
    """
...
def main() -> int:
    ...
    # Fixed 0.01-lot sizing is only a ceiling. The base intent still must pass
    # state, score, coverage, fresh quote, SL/TP, live RR and entry-drift gates.
    # Broker-native sizing can only reduce risk; it cannot raise the lot cap.
    ...
    print(
        "CTRADER_DEMO_BOUNDED_EXECUTION_POLICY "
