...
from .demo_xau_m15_liquidity_sweep_fade import (
    ...
    TP1_R,
    TP2_R,
    build_xau_m15_liquidity_sweep_fade_analysis,
...
UTC = timezone.utc
WORKER_NAME = "ctrader_demo_xau_m15_liquidity_sweep_fade_candidate_producer"
MARKER_EVENT = "DEMO_XAU_M15_LIQUIDITY_SWEEP_FADE_SIGNAL_EMITTED"


def _subset_cfg(cfg: ProjectConfig) -> ProjectConfig:
    if SYMBOL not in cfg.pair_map:
        raise SystemExit("XAU_M15_SWEEP_FADE_CONFIG_MISSING")
...
def _with_history_requirements(cfg: ProjectConfig) -> ProjectConfig:
    ...
    minimum_bars["M15"] = max(80, int(minimum_bars.get("M15", 0)))
...
def _record_marker(store, *, signal_id: str, signal, analysis) -> None:
    ...
    if not account_id:
        raise SystemExit("CTRADER_ACCOUNT_ID_REQUIRED_FOR_XAU_M15_SWEEP_FADE_MARKER")
    if signal.direction not in {"LONG", "SHORT"}:
        raise SystemExit("XAU_M15_SWEEP_FADE_MARKER_DIRECTION_INVALID")
    ...
    store.record_order_event(
        ...
        broker_order_id=f"XAU_M15_SWEEP_FADE:{signal_id}",
...
class XauM15LiquiditySweepFadeProducer(CTraderSignalProducer):
    last_signal = None
...
def run() -> int:
    ...
    if (
        ...
        raise SystemExit("XAU_M15_SWEEP_FADE_DEMO_ONLY")
    ...
    producer = XauM15LiquiditySweepFadeProducer(
        cfg,
        feed,
        store,
