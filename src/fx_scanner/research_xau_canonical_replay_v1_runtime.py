...
from .research_xau_canonical_replay_v1 import (
    ARTIFACT_CONTRACT,
    PIP_SIZE,
    RESEARCH_VERSION,
    SYMBOL,
...
UTC = timezone.utc
WORKER_NAME = "ctrader_xau_canonical_replay_v1"
HISTORY_BARS = 50_000


def _artifact_path() -> Path:
    raw = os.getenv(
        "XAU_CANONICAL_REPLAY_EVIDENCE_OUTPUT",
        "artifacts/xau-canonical-replay-v1.json",
    ).strip()
    if not raw:
        raise SystemExit("XAU_CANONICAL_REPLAY_EVIDENCE_OUTPUT_REQUIRED")
    return Path(raw)
...
def run() -> int:
    ...
    if str(policy.ctrader.get("environment", "")).upper() != "DEMO":
        raise SystemExit("XAU_CANONICAL_REPLAY_DEMO_ONLY")
    if not bool(policy.ctrader.get("require_demo", False)):
        raise SystemExit("XAU_CANONICAL_REPLAY_REQUIRE_DEMO")
    if SYMBOL not in {pair.symbol for pair in cfg.pairs}:
        raise SystemExit("XAU_CANONICAL_REPLAY_SYMBOL_NOT_CONFIGURED")
    ...
    if len(bars) < MIN_HISTORY_BARS:
        ...
        print(f"CTRADER_XAU_CANONICAL_REPLAY stage=DATA_INSUFFICIENT artifact={artifact}")
        return 0
    ...
    details["decision"] = replay_canonical(
        bars,
        costs=base_costs,
    ...
    approx = decision["outcomes"]["CANONICAL_APPROX"]["base"]
    print(
        "CTRADER_XAU_CANONICAL_REPLAY "
        f"bars={len(bars)}/{HISTORY_BARS} spread_pips={float(spread['median_pips']):.4f} "
        f"signals={decision['execution_ready_signals']} "
        f"coverage={decision['daily_coverage']['signal_day_coverage']:.4f} "
        f"approx_pf={approx.get('profit_factor')} approx_exp_r={approx.get('expectancy_r')} "
