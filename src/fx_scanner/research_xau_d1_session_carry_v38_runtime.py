...
from .research_xau_d1_session_carry_v38 import (
    ARTIFACT_CONTRACT,
    RESEARCH_VERSION,
    evaluate_v38,
)
...
UTC = timezone.utc
WORKER_NAME = "dukascopy_xau_d1_session_carry_v38"


def run() -> int:
    era_id = os.environ.get("V38_ERA", "").strip()
    if era_id not in ERAS:
        raise SystemExit(f"V38_ERA_INVALID:{era_id}")
    era = ERAS[era_id]
    bars = _fetch(era)
    decision = evaluate_v38(
        bars,
        era_id=era_id,
        era_start=era["start"],
        era_end=era["end"],
    ...
    path = Path(
        os.getenv(
            "V38_EVIDENCE_OUTPUT",
            f"artifacts/xau-d1-session-carry-v38-{era_id}.json",
        )
    )
    ...
    print(
        f"V38_RESULT era={era_id} rows={len(bars)} "
    ...
    for cost_id, scenario in decision["scenario_results"].items():
        for variant_id, payload in scenario["variants"].items():
            print(
                f"V38_VARIANT era={era_id} cost={cost_id} id={variant_id} "
        ...
        if cost_id == "V24_STRESS_4675":
            for variant_id, payload in scenario["variants"].items():
                for maturity, diag in payload["diagnostics"]["maturity"].items():
                    print(
                        f"V38_DIAG_MATURITY era={era_id} id={variant_id} "
                        f"bucket={maturity} {_metric_line(diag['metrics'])} "
