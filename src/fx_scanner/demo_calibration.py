...
def apply_demo_calibration_threshold(cfg):
    ...
    states = dict(cfg.scoring["states"]); states["execution_candidate_min"] = demo_min
    scoring = dict(cfg.scoring); scoring["states"] = states
...
def apply_demo_calibration_risk(cfg, *, max_risk_pct: float = DEMO_RISK_CEILING_PCT):
    ...
    ceiling = min(DEMO_RISK_CEILING_PCT, max(canonical_ceiling, requested if raw else canonical_ceiling))
    if not isfinite(requested) or not 0.0 < requested <= ceiling:
        raise SystemExit("CTRADER_DEMO_RISK_PER_TRADE_OUT_OF_RANGE")
    risk = dict(cfg.risk); risk["risk_per_trade_pct"] = requested; risk["max_risk_per_trade_pct"] = ceiling
...
def apply_demo_deep_analysis_top(cfg):
    ...
    if not raw: return cfg
    try: requested = int(raw)
    except ValueError as exc: raise SystemExit("CTRADER_DEMO_DEEP_ANALYSIS_TOP_INVALID") from exc
    canonical = int(cfg.strategy["selection"]["deep_analysis_top"]); universe = len(cfg.pairs)
    if not canonical <= requested <= 10: raise SystemExit("CTRADER_DEMO_DEEP_ANALYSIS_TOP_OUT_OF_RANGE")
    effective = min(requested, universe)
    if effective <= 0: raise SystemExit("CTRADER_DEMO_DEEP_ANALYSIS_TOP_OUT_OF_RANGE")
    selection = dict(cfg.strategy["selection"]); selection["deep_analysis_top"] = effective
    strategy = dict(cfg.strategy); strategy["selection"] = selection
    return replace(cfg, strategy=strategy)


def apply_demo_calibration_policy_risk(policy, *, max_risk_pct: float = DEMO_RISK_CEILING_PCT):
    ...
    demo_safety = dict(policy.demo_safety); demo_safety["max_risk_pct"] = ceiling
...
def build_demo_calibration_store(*, execution_ready_score_floor: float):
    ...
    store = SupabaseOperationalStore.from_env(execution_ready_score_floor=max(65.0, floor))
