...
def apply_demo_calibration_risk(cfg, *, max_risk_pct: float = 3.0):
    """Apply an explicit process-local DEMO risk target.

    Canonical configuration remains unchanged. An explicit
    CTRADER_DEMO_RISK_PER_TRADE_PCT may raise the calibration ceiling up to the
    DEMO-only wrapper ceiling, currently 3%, after environment checks.
    """
    canonical_ceiling = float(max_risk_pct)
    if not isfinite(canonical_ceiling) or not 0.0 < canonical_ceiling <= 3.0:
    ...
    ceiling = min(3.0, max(canonical_ceiling, requested if raw else canonical_ceiling))
...
def apply_demo_calibration_policy_risk(policy, *, max_risk_pct: float = 3.0):
    """Raise only the already-validated DEMO process risk ceiling."""
    ceiling = float(max_risk_pct)
    if not isfinite(ceiling) or not 0.0 < ceiling <= 3.0:
