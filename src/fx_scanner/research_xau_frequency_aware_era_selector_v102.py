...
from .research_xau_state_strategy_matrix_v101 import (
    EXPANSION_SPECIES,
    _broad_expansion,
    _species_side,
)

RESEARCH_VERSION="XAU_FREQUENCY_AWARE_ERA_SELECTOR_V102"
ARTIFACT_CONTRACT="XAU_FREQUENCY_AWARE_ERA_SELECTOR_V102_EVIDENCE_1"
POLICY_EFFECT="SHADOW_ONLY"
EXECUTION_INFLUENCE=False
PROMOTION_ELIGIBLE=False

LOW_FREQUENCY_STRATEGIES=("D1_STAGGERED",)
HIGH_FREQUENCY_STRATEGIES=("M15_L12","M15_L20")
...
def gate_frequency_aware(
    ...
    allow_broad_fallback=strategy_name in LOW_FREQUENCY_STRATEGIES
    kept=[]
    tier_counts:dict[str,int]={}
    state_checks:dict[str,dict[str,int]]={}
    selected_tier_by_state:dict[str,Any]={}
    ...
    for trade in ordered:
        ...
        elif allow_broad_fallback and len(broad_recent)>=MIN_COMPLETED_TRADES:
            tier="BROAD_RECENT_LOW_FREQUENCY_ONLY"
    ...
    return tuple(kept),{
        ...
        "frequency_class":"LOW" if allow_broad_fallback else "HIGH",
        "candidate_trades":len(ordered),
        "kept_trades":len(kept),
        "tier_counts":tier_counts,
        "state_checks":state_checks,
        "last_decision_by_state":selected_tier_by_state,
        "broad_fallback_allowed":allow_broad_fallback,
...
def _selected_by_strategy_era(selected:Mapping[str,Sequence[TournamentTrade]],a,b)->dict[str,Any]:
    return {
        name:_metrics(_period(trades,a,b))
...
def evaluate_v102(
    ...
    for name in SATELLITES:
        selected[name],gates[name]=gate_frequency_aware(
    ...
    return {
        ...
        "contract":{
            "low_frequency_strategies":list(LOW_FREQUENCY_STRATEGIES),
            "high_frequency_strategies":list(HIGH_FREQUENCY_STRATEGIES),
            "low_frequency_broad_fallback":True,
            "high_frequency_broad_fallback":False,
            "exact_recent_and_exact_epoch_available_to_all":True,
            "health_thresholds_unchanged":True,
            "calendar_year_used":False,
            "threshold_grid_search":False,
            "exploratory_after_v101":True,
        },
        ...
        "eras":era_out,
        "note":"V102 changes only the evidence-backoff topology by strategy frequency. High-frequency M15 families must prove exact-state health; low-frequency D1 staggered may temporarily back off to broader same-direction expansion health. No PF/expectancy/sample threshold changes.",
    }
