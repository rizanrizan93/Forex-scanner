...
from .demo_xau_m15_ema_smc_reclaim_execution import (
    ...
    TP1_R,
    TP2_R,
    build_xau_m15_ema_smc_reclaim_analysis,
...
UTC = timezone.utc
WORKER_NAME = "ctrader_demo_xau_m15_ema_smc_reclaim_candidate_producer"
MARKER_EVENT = "DEMO_XAU_M15_EMA_SMC_RECLAIM_SIGNAL_EMITTED"


def _subset_cfg(cfg: ProjectConfig) -> ProjectConfig:
    if SYMBOL not in cfg.pair_map:
        raise SystemExit("XAU_M15_EMA_SMC_RECLAIM_CONFIG_MISSING")
...
def _with_history_requirements(cfg: ProjectConfig) -> ProjectConfig:
    ...
    minimum_bars["H1"] = max(60, int(minimum_bars.get("H1", 0)))
...
def _record_marker(store, *, signal_id: str, signal, analysis) -> None:
    ...
    if not account_id:
        raise SystemExit("CTRADER_ACCOUNT_ID_REQUIRED_FOR_XAU_M15_EMA_SMC_RECLAIM_MARKER")
    if signal.direction not in {"LONG", "SHORT"}:
        raise SystemExit("XAU_M15_EMA_SMC_RECLAIM_MARKER_DIRECTION_INVALID")
    ...
    payload = {
        **signal.evidence(),
        ...
        "user_authorized_direct_demo_execution": True,
        "broker_tp1_r": TP1_R,
        "broker_tp1": None if plan is None else plan.tp1,
        "broker_tp2_r": TP2_R,
        "broker_tp2": None if plan is None else plan.tp2,
    }
    store.record_order_event(
        backend="CTRADER",
        account_id=account_id,
        signal_key=str(signal_id),
        broker_order_id=f"XAU_M15_EMA_SMC_RECLAIM:{signal_id}",
        event_type=MARKER_EVENT,
        accepted=True,
        code=STRATEGY_ID,
        message="user-authorized XAU M15 EMA-SMC reclaim DEMO candidate emitted",
        payload=payload,
    )


class XauM15EmaSmcReclaimProducer(CTraderSignalProducer):
    last_signal = None
    ...
    def _bar_window(self, timeframe: str, count: int, now: datetime) -> tuple[datetime, datetime]:
        ...
        factor = 1.65 if timeframe in {"D1", "H4", "H1"} else 1.20
    ...
    def run_once(self) -> SignalProducerReport:
        ...
        run_id = self.store.start_scanner_run(
            ...
            data_contract_version="XAU_M15_EMA_SMC_RECLAIM_V1_DEMO_EXECUTION",
            ...
            if signal.active and signal.execution_eligible:
                ...
                if not duplicate:
                    ...
                    try:
                        ...
                        selected = ()
                        failures[SYMBOL] = f"LIVE_GEOMETRY_BLOCK:{exc}"
            if duplicate:
                failures[SYMBOL] = "DUPLICATE_M15_EMA_SMC_RECLAIM_BAR_DIRECTION_BLOCKED"
...
def run() -> int:
    ...
    if (
        ...
        raise SystemExit("XAU_M15_EMA_SMC_RECLAIM_DEMO_ONLY")
    ...
    producer = XauM15EmaSmcReclaimProducer(
        cfg,
        feed,
        store,
    ...
    try:
        ...
        if report.execution_ready:
            ...
            if len(ready) != 1 or producer.last_signal is None or not analyses.get(SYMBOL):
                raise SystemExit("XAU_M15_EMA_SMC_RECLAIM_READY_CARDINALITY_INVALID")
            _record_marker(
                store,
        ...
        score = 0.0 if producer.last_signal is None else float(producer.last_signal.score)
        ...
        print(
            "CTRADER_DEMO_XAU_M15_EMA_SMC_RECLAIM_OK "
            f"direction={direction or 'NONE'} score={score:.2f} signals={report.signals_written} "
            f"ready={report.execution_ready} geometry={geometry_written} "
            f"reason={reason or 'NONE'} demo_execution_authorized=1"
        )
        return 0
    finally:
