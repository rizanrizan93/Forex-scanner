...
from .research_xau_competing_zone_v175 import (
    ARTIFACT_CONTRACT,
    RESEARCH_VERSION,
    evaluate_competing_zone_research,
)
from .research_xau_m15_dual_strategy_runtime import _fetch_history, _history_target
from .storage.supabase_operational import SupabaseOperationalStore

SYMBOL = "XAUUSD"
WORKER_NAME = "ctrader_xau_competing_zone_ranker_v175"


def _artifact_path() -> Path:
    raw = os.getenv(
        "XAU_COMPETING_ZONE_V175_OUTPUT",
        "artifacts/xau-competing-zone-v175.json",
    ).strip()
    if not raw:
        raise SystemExit("XAU_COMPETING_ZONE_V175_OUTPUT_REQUIRED")
    return Path(raw)
...
def run() -> int:
    ...
    if str(policy.ctrader.get("environment", "")).upper() != "DEMO":
        raise SystemExit("XAU_COMPETING_ZONE_V175_DEMO_ONLY")
    if not bool(policy.ctrader.get("require_demo", False)):
        raise SystemExit("XAU_COMPETING_ZONE_V175_REQUIRE_DEMO")
    if SYMBOL not in cfg.pair_map:
        raise SystemExit("XAU_COMPETING_ZONE_V175_SYMBOL_NOT_CONFIGURED")
