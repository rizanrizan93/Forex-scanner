...
from .research_xau_d1_regime_h1_session_v11 import (
    ARTIFACT_CONTRACT,
    RESEARCH_VERSION,
    SYMBOL,
    evaluate_d1_regime_h1_session_v11,
)
...
def run() -> int:
    ...
    if str(policy.ctrader.get("environment", "")).upper() != "DEMO":
        raise SystemExit("XAU_V11_DEMO_ONLY")
    if not bool(policy.ctrader.get("require_demo", False)):
        raise SystemExit("XAU_V11_REQUIRE_DEMO")
    if SYMBOL not in {pair.symbol for pair in cfg.pairs}:
        raise SystemExit("XAU_V11_SYMBOL_NOT_CONFIGURED")
    ...
    if insufficient:
        details["decision"] = {
            ...
            "reason": (
                f"H1_HISTORY_INCOMPLETE:{len(bars)}"
                if not bool(spread.get("available"))
                else f"H1_HISTORY_INCOMPLETE:{len(bars)}:source_exhausted={int(source_exhausted)}"
            ),
        ...
        details["decision"] = evaluate_d1_regime_h1_session_v11(
            bars,
            costs=base_costs,
    ...
    path = Path(
        os.getenv(
            "XAU_V11_EVIDENCE_OUTPUT",
            "artifacts/xau-d1-regime-h1-session-v11.json",
        )
    )
    ...
    if decision.get("stage") == "DATA_INSUFFICIENT":
        print(f"CTRADER_XAU_V11 stage=DATA_INSUFFICIENT artifact={path}")
    else:
        print(
            "CTRADER_XAU_V11 "
            f"bars={len(bars)}/{HISTORY_TARGET} "
