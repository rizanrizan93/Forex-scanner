...
from .research_xau_stale_reentry_probation_v106 import ARTIFACT_CONTRACT,RESEARCH_VERSION,evaluate_v106
...
UTC=timezone.utc
WORKER_NAME="dukascopy_xau_stale_reentry_probation_v106"
...
def run()->int:
    ...
    p=Path(os.getenv("V106_EVIDENCE_OUTPUT","artifacts/xau-stale-reentry-probation-v106.json"));p.parent.mkdir(parents=True,exist_ok=True)
    ...
    for k,m in d["full_period"].items(): print(f"V106_FULL id={k} {_metric_line(m)}")
    for era,x in d["eras"].items():
        for k in ("core","selector"):
            print(f"V106_ERA id={k} era={era} {_metric_line(x[k])}")
            c=x["cash_fresh_100"][k]
            print(f"V106_CASH id={k} era={era} ending={c['ending_balance_usd']} return_pct={c['return_pct']} opened={c['opened_trades']} ruin={c['ruin']}")
        for s,m in x["selected_strategy_contribution"].items():
            print(f"V106_CONTRIB strategy={s} era={era} {_metric_line(m)}")
    for k,g in d["strategy_state_gates"].items(): print("V106_GATE id="+k+" "+json.dumps(g,sort_keys=True,default=str))
    return 0
