...
from .research_xau_m15_forward_witness_v108 import ARTIFACT_CONTRACT,RESEARCH_VERSION,evaluate_v108
...
UTC=timezone.utc
WORKER_NAME="dukascopy_xau_m15_forward_witness_v108"
...
def run()->int:
    ...
    p=Path(os.getenv("V108_EVIDENCE_OUTPUT","artifacts/xau-m15-forward-witness-v108.json"));p.parent.mkdir(parents=True,exist_ok=True)
    ...
    for k,m in d["full_period"].items(): print(f"V108_FULL id={k} {_metric_line(m)}")
    for era,x in d["eras"].items():
        for k in ("core","selector"):
            print(f"V108_ERA id={k} era={era} {_metric_line(x[k])}")
            c=x["cash_fresh_100"][k]
            print(f"V108_CASH id={k} era={era} ending={c['ending_balance_usd']} return_pct={c['return_pct']} opened={c['opened_trades']} ruin={c['ruin']}")
        for s,m in x["selected_strategy_contribution"].items():
            print(f"V108_CONTRIB strategy={s} era={era} {_metric_line(m)}")
    for k,g in d["strategy_state_gates"].items(): print("V108_GATE id="+k+" "+json.dumps(g,sort_keys=True,default=str))
    return 0
