...
from .research_xau_us_short_continuation_v2 import (
    ...
    RESEARCH_VERSION,
    SYMBOL,
    evaluate_us_short_v2,
)
from .storage.supabase_operational import SupabaseOperationalStore

UTC = timezone.utc
WORKER_NAME = "ctrader_xau_us_short_continuation_v2"
HISTORY_BARS = 100_000


def _artifact_path() -> Path:
    raw = os.getenv(
        "XAU_US_SHORT_V2_EVIDENCE_OUTPUT",
        "artifacts/xau-us-short-continuation-v2.json",
    ).strip()
    if not raw:
        raise SystemExit("XAU_US_SHORT_V2_EVIDENCE_OUTPUT_REQUIRED")
    return Path(raw)
...
def run() -> int:
    ...
    if str(policy.ctrader.get("environment", "")).upper() != "DEMO":
        raise SystemExit("XAU_US_SHORT_V2_DEMO_ONLY")
    if not bool(policy.ctrader.get("require_demo", False)):
        raise SystemExit("XAU_US_SHORT_V2_REQUIRE_DEMO")
    if SYMBOL not in {pair.symbol for pair in cfg.pairs}:
        raise SystemExit("XAU_US_SHORT_V2_SYMBOL_NOT_CONFIGURED")
    ...
    details: dict[str, Any] = {
        ...
        "pip_size": PIP_SIZE,
        "hypothesis": (
            "US-session SHORT continuation showed the only material positive "
            "development expectancy in V1 direction diagnostics; V2 tests that "
            "hypothesis on a longer sample with preregistered perturbations."
        ),
    }
    ...
    if len(bars) < MIN_HISTORY_BARS:
        ...
        print(f"CTRADER_XAU_US_SHORT_V2 stage=DATA_INSUFFICIENT artifact={artifact}")
        return 0
    ...
    details["decision"] = evaluate_us_short_v2(
        bars,
        costs=base_costs,
